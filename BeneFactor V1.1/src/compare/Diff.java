...
public class Diff {
	 ...
	 */
	private StringBuffer diff_information = new StringBuffer(); 
	
	public static String getDiffDescription(String path1, String path2)
	{
		Diff diff = new Diff();
		diff.doDiff(path1, path2);
		return diff.diff_information.toString();
	}
	...
	void inputscan(fileInfo pinfo) throws IOException {
		...
		BufferedReader bReader
         = new BufferedReader(new InputStreamReader(pinfo.file));
		 
		while ((linebuffer = bReader.readLine()) != null) {
...
class node { /* the tree is made up of these nodes */
	...
	Diff diff;
	/* symtab is a tree hung from this */
	/**
	 ...
	 *            created the node.
	 */
	static node matchsymbol(String pline, Diff d) {
		int comparison;
		node pnode = panchor;
		if (panchor == null)
			return panchor = new node(pline, d);
		for (;;) {
	 ...
	 */
	static node addSymbol(String pline, boolean inoldfile, int linenum, Diff d) {
		node pnode;
		pnode = matchsymbol(pline, d); /* find the node in the tree */
